...
function(input, output, session) {
  ...
  ## Output for Tab 4
  ## ---------------------------
  output$ListAll <- DT::renderDataTable(tabAll, options = list(pageLength = 6))
  
  tsData <- reactive({
    # Cell <- input$ListAll_cell_clicked
    # return list(row = row_index, col = column_index, value = cell_value)
    # CellID <- Cell$value
    # haven't figured out how cell_clicked works yet...
    CellID <- input$ListAll_search
    id <- which(ID535 %in% CellID)
    data <- curve535[id, ]
  })
